...
class Edit extends Component
{
    ...
    {
        abort_unless(
            auth()->user()?->can('pages.edit') ?? false,
            403,
        ...
        $this->template = (string) ($page->template ?? Page::TEMPLATE_DEFAULT);
        $this->showInMenu = (bool) $page->show_in_menu;
        $this->sortOrder = (int) $page->sort_order;

        $default = Language::query()->default()->first()
            ?? Language::query()->active()->ordered()->first();

        $this->defaultLanguageId = $default?->id;
    ...
    private function loadTranslationsFromModel(): void
    {
        ...
        foreach ($this->page->translations as $row) {
            $this->translations[(int) $row->language_id] = [
        ...
        // Ensure the default language is at least present as an empty tab.
        if ($this->defaultLanguageId !== null && ! isset($this->translations[$this->defaultLanguageId])) {
            $this->translations[$this->defaultLanguageId] = $this->blankRow();
        }
    }
    ...
    public function removeTranslation(int $languageId): void
    {
        ...
        // Mark for deletion via UpdatePageAction's translations API.
    ...
    public function save(UpdatePageAction $update): void
    {
        ...
        try {
            $update->handle($this->page, $this->buildPayload());
    ...
    public function publish(PublishPageAction $publish): void
    {
        $this->authorize('pages.publish');

        try {
            $publish->handle($this->page);
            $this->page = $this->page->fresh(['translations']);
    ...
    public function archive(ArchivePageAction $archive): void
    {
        $this->authorize('pages.publish');

        try {
            $archive->handle($this->page);
            $this->page = $this->page->fresh(['translations']);
            $this->status = $this->page->status->value;
    ...
    /**
     * @return list<array{id:int, code:string, name:string, flag:?string, active:bool, percent:int, is_default:bool, is_published:bool, deleted:bool}>
     */
    #[Computed]
