    ...
    @if ($this->featured->isNotEmpty())
            <section class="mx-auto max-w-7xl px-4 pt-6 pb-12">
                <p class="mb-4 text-[11px] font-bold uppercase tracking-[0.24em] text-slate-400 dark:text-slate-500">
                    Yesterday's edition · {{ now()->translatedFormat('F j, Y') }}
                </p>
                    ...
                    @if ($heroT && $heroSlug)
                                    <article class="group md:col-span-2">
                                        <div class="flex flex-wrap items-center gap-2 text-[10px] font-bold uppercase tracking-[0.22em]">
                                            @if ($hero->is_breaking)
                                                <span class="inline-flex items-center gap-1 rounded-md bg-rose-600 px-2 py-0.5 text-white">
                                                    <span class="h-1.5 w-1.5 animate-pulse rounded-full bg-white"></span>
                                                    Heart Breaking
                                                </span>
                                            @endif
                                            @if ($hero->category)
                                                <a href="{{ route('frontend.category', ['locale' => $locale?->code, 'slug' => $hero->category->translate('slug')]) }}"
                                                    class="text-slate-600 hover:text-emerald-700 dark:text-slate-400">
                                        ...
                                        @if ($heroT->excerpt)
                                            <p class="mt-3 max-w-prose text-sm leading-relaxed text-slate-600 md:text-base dark:text-slate-400">
                                                {{ $heroT->excerpt }}
                                            </p>
                                        @endif
                                        ...
                                        <div class="mt-3 flex flex-wrap items-center gap-2 text-[11px] text-slate-500 dark:text-slate-400">
                                            @if ($hero->author?->name)
                                                <span class="font-semibold text-slate-700 dark:text-slate-300">{{ $hero->author->name }}</span>
                        ...
                        $heroHasImage = $hf && ($hf->isImage() || ($hf->path !== null && $hf->path !== ''));
                                        @endphp
                                        <a href="{{ $heroUrl }}"
                                            class="mt-4 block aspect-[16/9] overflow-hidden rounded-xl bg-slate-100 dark:bg-slate-800">
                                            @if ($heroHasImage)
                                                <img src="{{ $hf->url() }}" alt="{{ $hf->alt_text ?? $heroT->title }}" loading="eager"
                                                    class="h-full w-full object-cover transition duration-700 group-hover:scale-105">
                                            @else
                                                <img src="https://picsum.photos/seed/np{{ $hero->id }}/1280/720" alt="{{ $heroT->title }}"
                                                    loading="eager"
                                                    class="h-full w-full object-cover transition duration-700 group-hover:scale-105">
                                            @endif
                                        </a>
                    ...
                    <div class="mt-5 space-y-5">
                        ...
                        <div
                            ...
                            <div class="min-w-0 flex-1">
                                ...
                                <p class="mt-1 text-[10px] text-slate-500">{{ $tp->published_at?->diffForHumans() }}</p>
                            </div>
                        </div>
                        @endforeach
                    </div>
