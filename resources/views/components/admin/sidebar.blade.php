...
<aside data-sidebar class="fixed inset-y-0 left-0 z-40 flex w-60 -translate-x-full flex-col border-r border-white/10 bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950 text-slate-100 shadow-2xl shadow-slate-950/40 transition-all duration-300 lg:sticky lg:translate-x-0">
    @php
    ...
    <div class="flex h-16 items-center justify-between border-b border-white/10 px-4">
        <a href="{{ route('dashboard') }}" class="flex items-center gap-3">
            @if ($logoLightUrl)
                <img src="{{ $logoLightUrl }}" alt="{{ $companyName }}" class="h-[60px] w-[100px] rounded-xl object-contain {{ $logoDarkUrl && $logoDarkUrl !== $logoLightUrl ? 'dark:hidden' : '' }}">
                @if ($logoDarkUrl && $logoDarkUrl !== $logoLightUrl)
                    <img src="{{ $logoDarkUrl }}" alt="{{ $companyName }}" class="hidden h-10 w-10 rounded-xl object-contain dark:block">
                @endif
            @else
                <span class="grid h-10 w-10 place-items-center rounded-xl bg-gradient-to-br from-indigo-500 to-sky-500 text-sm font-bold text-white shadow-lg shadow-indigo-900/40">{{ strtoupper(substr($companyName, 0, 1)) }}</span>
            @endif
            {{-- <span data-sidebar-label class="text-sm font-semibold tracking-wide text-white">{{ $companyName }}</span> --}}
        </a>

        <button type="button" class="rounded-xl border border-white/10 bg-white/5 p-2 text-slate-300 transition hover:bg-white/10 hover:text-white lg:hidden" data-sidebar-close aria-label="Close sidebar">
    ...
    <div class="flex-1 overflow-y-auto px-3 py-4">
        <nav class="space-y-1.5">
            ...
            >
                ...
                <span data-sidebar-label class="truncate">My Dashboard</span>
            </a>
            ...
            <div>
                <button
                    ...
                    data-sidebar-menu-toggle="dashboards-sidebar-menu"
                    aria-controls="dashboards-sidebar-menu"
                >
                ...
                <div data-sidebar-submenu="dashboards-sidebar-menu" class="{{ $subMenuWrapper }} {{ $dashboardsActive ? '' : 'hidden' }}">
            ...
            @canany(['staff.view', 'departments.view'])
                ...
                <div>
                    <button
                        ...
                        data-sidebar-menu-toggle="team-sidebar-menu"
                        aria-controls="team-sidebar-menu"
                    >
                        <span class="{{ $menuContentClass }}">
                            <i data-lucide="users-round" class="h-4 w-4 shrink-0"></i>
                            <span data-sidebar-label class="truncate">Team</span>
                        </span>
                        <i data-lucide="chevron-down" class="{{ $menuChevronClass }}" data-sidebar-label></i>
                    </button>

                    <div data-sidebar-submenu="team-sidebar-menu" class="{{ $subMenuWrapper }} {{ $teamMenuActive ? '' : 'hidden' }}">
                        @can('staff.view')
                            <a href="{{ route('admin.staff.index') }}" class="{{ $childMenuBaseClass }} {{ request()->routeIs('admin.staff.*') ? $childMenuActiveClass : $childMenuInactiveClass }}">
                                <span data-sidebar-label>Staff</span>
                            </a>
                        @endcan
                        @can('departments.view')
                            <a href="{{ route('admin.departments.index') }}" class="{{ $childMenuBaseClass }} {{ request()->routeIs('admin.departments.*') ? $childMenuActiveClass : $childMenuInactiveClass }}">
            ...
            {{-- ============================================================
                 NewsPilot AI — Content + Editorial menu groups
                 ============================================================ --}}

            {{-- Content (Posts, News, Categories, Tags, Pages, Media, RSS) --}}
            @canany([
                'posts.view', 'posts.view_any', 'news.view',
                'categories.view', 'tags.view', 'pages.view', 'media.view', 'rss.view',
            ])
                @php($contentMenuActive = (
                ...
                <div>
                    <button
                        ...
                        data-sidebar-menu-toggle="content-sidebar-menu"
                        aria-controls="content-sidebar-menu"
                    >
                        <span class="{{ $menuContentClass }}">
                    ...
                    <div data-sidebar-submenu="content-sidebar-menu" class="{{ $subMenuWrapper }} {{ $contentMenuActive ? '' : 'hidden' }}">
                        @canany(['posts.view', 'posts.view_any'])
                            <a href="{{ route('admin.posts.index') }}" wire:navigate class="{{ $childMenuBaseClass }} {{ request()->routeIs('admin.posts.*') ? $childMenuActiveClass : $childMenuInactiveClass }}">
                                <span data-sidebar-label>Posts</span>
                            </a>
                        @endcanany
                        @can('news.view')
                            <a href="{{ route('admin.posts.index', ['type' => 'news']) }}" wire:navigate class="{{ $childMenuBaseClass }} {{ request()->routeIs('admin.news.*') ? $childMenuActiveClass : $childMenuInactiveClass }}">
                                <span data-sidebar-label>News</span>
                            </a>
                        @endcan
                        @can('categories.view')
                            <a href="{{ route('admin.categories.index') }}" wire:navigate class="{{ $childMenuBaseClass }} {{ request()->routeIs('admin.categories.*') ? $childMenuActiveClass : $childMenuInactiveClass }}">
                                <span data-sidebar-label>Categories</span>
                            </a>
                        @endcan
                        @can('tags.view')
                            <a href="{{ route('admin.tags.index') }}" wire:navigate class="{{ $childMenuBaseClass }} {{ request()->routeIs('admin.tags.*') ? $childMenuActiveClass : $childMenuInactiveClass }}">
                                <span data-sidebar-label>Tags</span>
                            </a>
                        @endcan
                        @can('pages.view')
                            <a href="{{ route('admin.pages.index') }}" wire:navigate class="{{ $childMenuBaseClass }} {{ request()->routeIs('admin.pages.*') ? $childMenuActiveClass : $childMenuInactiveClass }}">
                                <span data-sidebar-label>Pages</span>
                            </a>
                        @endcan
                        @can('media.view')
                            <a href="{{ route('admin.media.index') }}" wire:navigate class="{{ $childMenuBaseClass }} {{ request()->routeIs('admin.media.*') ? $childMenuActiveClass : $childMenuInactiveClass }}">
            ...
            @canany(['ai.use', 'ai.use_writer', 'ai.use_seo', 'ai.settings', 'ai.reports'])
                ...
                <div>
                    <button
                        ...
                        data-sidebar-menu-toggle="ai-sidebar-menu"
                        aria-controls="ai-sidebar-menu"
                    >
                        <span class="{{ $menuContentClass }}">
                    ...
                    <div data-sidebar-submenu="ai-sidebar-menu" class="{{ $subMenuWrapper }} {{ $aiMenuActive ? '' : 'hidden' }}">
                        @can('ai.use_writer')
            ...
            @canany([
                'editorial.review_queue', 'editorial.calendar', 'editorial.approve',
                'comments.moderate',
            ])
                ...
                <div>
                    <button
                        ...
                        data-sidebar-menu-toggle="editorial-sidebar-menu"
                        aria-controls="editorial-sidebar-menu"
                    >
                        <span class="{{ $menuContentClass }}">
                    ...
                    <div data-sidebar-submenu="editorial-sidebar-menu" class="{{ $subMenuWrapper }} {{ $editorialMenuActive ? '' : 'hidden' }}">
                        @can('editorial.review_queue')
                            @php($isQueueActive = request()->routeIs('admin.editorial.queue'))
                            <a href="{{ route('admin.editorial.queue') }}"
                               wire:navigate
                        ...
                        @can('editorial.revisions')
                            <a href="{{ route('admin.posts.index') }}" wire:navigate
                               class="{{ $childMenuBaseClass }} {{ $childMenuInactiveClass }}">
                                ...
                                <span data-sidebar-label class="ml-auto text-[10px] uppercase tracking-wider text-slate-400">per&nbsp;post</span>
                            </a>
                        @endcan
                    </div>
                </div>
            ...
            @canany(['seo.view', 'seo.manage', 'seo.audit', 'seo.sitemap', 'seo.redirects'])
                ...
                <div>
                    <button
                        ...
                        data-sidebar-menu-toggle="seo-sidebar-menu"
                        aria-controls="seo-sidebar-menu"
                    >
                        <span class="{{ $menuContentClass }}">
                    ...
                    <div data-sidebar-submenu="seo-sidebar-menu" class="{{ $subMenuWrapper }} {{ $seoActive ? '' : 'hidden' }}">
                        @can('seo.view')
            ...
            {{-- Settings --}}
            @canany(['settings.view', 'settings.update', 'settings.roles', 'settings.permissions', 'logs.login.view', 'logs.activity.view'])
                @php($settingsMenuActive = (
                    request()->routeIs('settings')
                    || request()->routeIs('settings.*')
                    || request()->routeIs('admin.settings.*')
                    || request()->routeIs('admin.permission-groups.index')
                    ...
                    || request()->routeIs('admin.assign-role.index')
                    || request()->routeIs('admin.assign-user-permissions.index')
                    || request()->routeIs('admin.assign-user-permissions.edit')
                ...
                <div>
                    <button
                        ...
                        data-sidebar-menu-toggle="settings-sidebar-menu"
                    ...
                    <div data-sidebar-submenu="settings-sidebar-menu" class="{{ $subMenuWrapper }} {{ $settingsMenuActive ? '' : 'hidden' }}">
                        @can('settings.view')
                            <a href="{{ route('admin.settings.index') }}" class="{{ $childMenuBaseClass }} {{ request()->routeIs('settings') || request()->routeIs('admin.settings.*') ? $childMenuActiveClass : $childMenuInactiveClass }}">
                                <span data-sidebar-label>General Settings</span>
                            </a>
                        @endcan
                        @canany(['settings.permissions', 'settings.roles'])
                            <div data-sidebar-label class="px-3 pt-1 text-[11px] font-semibold uppercase tracking-[0.14em] text-slate-500">
                                Permission Settings
                            </div>
                            @can('settings.permissions')
                                <a href="{{ route('admin.permission-groups.index') }}" class="{{ $childMenuBaseClass }} {{ request()->routeIs('admin.permission-groups.index') ? $childMenuActiveClass : $childMenuInactiveClass }}">
                                    <span data-sidebar-label>Permission Groups</span>
                                </a>
                                <a href="{{ route('admin.permissions.index') }}" class="{{ $childMenuBaseClass }} {{ request()->routeIs('admin.permissions.index') ? $childMenuActiveClass : $childMenuInactiveClass }}">
                                    <span data-sidebar-label>Permissions</span>
                                </a>
                                <a href="{{ route('admin.assign-user-permissions.index') }}" class="{{ $childMenuBaseClass }} {{ request()->routeIs('admin.assign-user-permissions.index') || request()->routeIs('admin.assign-user-permissions.edit') ? $childMenuActiveClass : $childMenuInactiveClass }}">
                                    <span data-sidebar-label>Assign Permissions</span>
                                </a>
                            @endcan
                            @can('settings.roles')
                                <a href="{{ route('admin.roles.index') }}" class="{{ $childMenuBaseClass }} {{ request()->routeIs('admin.roles.index') || request()->routeIs('admin.roles.create') || request()->routeIs('admin.roles.edit') ? $childMenuActiveClass : $childMenuInactiveClass }}">
                                    <span data-sidebar-label>Roles</span>
                                </a>
                                <a href="{{ route('admin.assign-role.index') }}" class="{{ $childMenuBaseClass }} {{ request()->routeIs('admin.assign-role.index') ? $childMenuActiveClass : $childMenuInactiveClass }}">
                        ...
                        @canany(['logs.login.view', 'logs.activity.view'])
                            ...
                            @can('logs.login.view')
                                <a href="{{ route('admin.logs.login.index') }}" wire:navigate class="{{ $childMenuBaseClass }} {{ request()->routeIs('admin.logs.login.*') ? $childMenuActiveClass : $childMenuInactiveClass }}">
                                    <span data-sidebar-label>Login Logs</span>
                                </a>
                            @endcan
                            @can('logs.activity.view')
                                <a href="{{ route('admin.logs.activity.index') }}" wire:navigate class="{{ $childMenuBaseClass }} {{ request()->routeIs('admin.logs.activity.*') ? $childMenuActiveClass : $childMenuInactiveClass }}">
    ...
    <div class="border-t border-white/10 p-4">
        <div class="flex items-center gap-3 rounded-2xl border border-white/10 bg-white/5 p-3">
            <img src="{{ auth()->user()?->avatarUrl() }}" alt="Avatar" class="h-10 w-10 rounded-full object-cover ring-2 ring-white/20">
            <div data-sidebar-label class="min-w-0">
                <div class="truncate text-sm font-semibold text-white">{{ auth()->user()?->name ?? 'Admin' }}</div>
                <div class="truncate text-xs text-slate-400">{{ auth()->user()?->email ?? 'admin@newspilot.ai' }}</div>
            </div>
        </div>
    </div>
</aside>
