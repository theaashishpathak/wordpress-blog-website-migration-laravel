...
<div wire:ignore x-data="tiptapEditor(@js($value), {})" x-init="init()" data-placeholder="{{ $placeholder }}"
    ...
    <div
        class="flex flex-wrap items-center gap-0.5 border-b border-slate-200 bg-slate-50/80 px-2 py-1.5 text-slate-600 dark:border-slate-800 dark:bg-slate-950/40 dark:text-slate-300">

        {{-- Block type --}}
        <button type="button" x-on:click="cmd('paragraph')" title="Paragraph" class="rt-btn">
            <span class="text-xs font-semibold">P</span>
        </button>
        <button type="button" x-on:click="cmd('h2')" title="Heading 2" x-bind:class="is('h2') && 'rt-btn-active'"
            class="rt-btn">
            <span class="text-xs font-bold">H2</span>
        </button>
        <button type="button" x-on:click="cmd('h3')" title="Heading 3" x-bind:class="is('h3') && 'rt-btn-active'"
            class="rt-btn">
            <span class="text-xs font-bold">H3</span>
        </button>
        <button type="button" x-on:click="cmd('h4')" title="Heading 4" x-bind:class="is('h4') && 'rt-btn-active'"
            class="rt-btn">
        ...
        {{-- Lists / quote --}}
        <button type="button" x-on:click="cmd('ul')" title="Bulleted list" x-bind:class="is('ul') && 'rt-btn-active'"
            class="rt-btn">
            <i data-lucide="list" class="h-3.5 w-3.5"></i>
        </button>
        <button type="button" x-on:click="cmd('ol')" title="Numbered list" x-bind:class="is('ol') && 'rt-btn-active'"
            class="rt-btn">
        ...
        <button type="button" x-on:click="cmd('unlink')" title="Remove link" class="rt-btn">
            <i data-lucide="link-2-off" class="h-3.5 w-3.5"></i>
        </button>
        <button type="button" x-on:click="cmd('image')" title="Insert image" class="rt-btn">
            <i data-lucide="image" class="h-3.5 w-3.5"></i>
        </button>
        <button type="button" x-on:click="cmd('youtube')" title="Embed YouTube" class="rt-btn">
            <i data-lucide="youtube" class="h-3.5 w-3.5"></i>
        </button>
        <button type="button" x-on:click="cmd('table')" title="Insert table" class="rt-btn">
            <i data-lucide="table" class="h-3.5 w-3.5"></i>
        </button>
        <button type="button" x-on:click="cmd('hr')" title="Horizontal rule" class="rt-btn">
        ...
        <button type="button" x-on:click="cmd('clear')" title="Clear formatting" class="rt-btn">
    ...
    <textarea x-show="sourceView" x-cloak x-bind:value="rawHtml" x-on:input="setRawHtml($event.target.value)"
    ...
    <textarea x-ref="textarea" wire:model.defer="{{ $model }}" hidden aria-hidden="true">{{ $value }}</textarea>
</div>
